...
p.store("gantry:notice",true);}switch(n){case"select":m.set("value",q);m.fireEvent("change");if(typeof jQuery!="undefined"){jQuery("#"+m.id).trigger("liszt:updated");
}break;case"input":var s=m.get("class");m.set("value",q);if(s.contains("picker-input")){document.getElement("[data-moorainbow-trigger="+m.id+"] .overlay").setStyle("background-color",q);
