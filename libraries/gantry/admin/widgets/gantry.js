...
if(a){var f=$$(document.getElements("#g4-toolbar .g4-actions > .rok-button").slice(0,-1));var e=document.id("template-form");var b=null;var d=new Request({url:GantryAjaxURL,method:"post",onRequest:function(){},onSuccess:function(h){growl.alert("Gantry",h,{duration:6000});
...
var b;a.each(function(f,d){var e=f.id.replace("_type","");b=document.getElement(".selectedset-enabler input[id^="+e+"]");f.store("gantry:values",f.getElements("option").get("value"));
f.addEvent("change",function(){this.retrieve("gantry:values").each(function(h){var g=document.id("set-"+h);if(g&&b.value.toInt()){g.removeClass("selectedset-hidden-field");
g.setStyle("display",(h==this.value)?"table-row-group":"none");if(window.selectboxes&&h==this.value){g.getElements(".selectbox-wrapper").each(function(i){i.getElements(".selectbox-top, .selectbox-dropdown").set("style","");
window.selectboxes.updateSizes(i);});}}},this);});});$$(".selectedset-enabler input[id]").each(function(e,d){e.store("gantry:values",a[d].retrieve("gantry:values"));
e.addEvent("onChange",function(){this.retrieve("gantry:values").each(function(g){var f=document.id("set-"+g);if(f){if(!this.value.toInt()){f.setStyle("display","none");
}else{f.removeClass("selectedset-hidden-field");f.setStyle("display",(g==a[d].get("value"))?"table-row-group":"none");}}},this);});});var c=document.id("jform_params_menu_type");
if(c){c.fireEvent("change");}},cleanance:function(){Gantry.overridesBadges();Gantry.tabs=[];Gantry.panels=[];var e=document.getElement(".pane-sliders")||document.getElement("#g4-panels");
...
this.setOptions(b);this.id=new Element("div",{id:"gantry-layer"}).inject(document.body);this.fx=new Fx.Tween(this.id,{duration:this.options.duration,wait:false,onStart:function(){this.id.setStyle("visibility","visible");
...
this.open=false;},show:function(){this.calcSizes();this.fx.start("opacity",this.options.opacity);},hide:function(){this.fireEvent("hide");this.fx.start("opacity",0);
},toggle:function(){this[this.open?"hide":"show"]();},calcSizes:function(){this.id.setStyles({width:window.getScrollSize().x,height:window.getScrollSize().y});
